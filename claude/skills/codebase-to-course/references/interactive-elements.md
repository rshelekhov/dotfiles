...
5. Message Flow / Data Flow Animation
6. Interactive Architecture Diagram
7. Layer Toggle Demo
8. "Spot the Bug" Challenge
9. Scenario Quiz
10. Callout Boxes
11. Pattern/Feature Cards
12. Flow Diagrams
13. Permission/Config Badges
...
window.checkQuiz = function(sectionId) {
  ...
  questions.forEach(q => {
    ...
    if (selected.dataset.value === correctValue) {
      ...
      feedback.textContent = 'Exactly! ' + getExplanation(q, true);
      feedback.className = 'quiz-feedback show success';
    } else {
      selected.classList.add('incorrect');
      q.querySelector('[data-value="' + correctValue + '"]').classList.add('correct');
      feedback.textContent = 'Not quite. ' + getExplanation(q, false);
...
zones.forEach(zone => {
  ...
  target.addEventListener('dragover', (e) => {
    e.preventDefault(); target.classList.add('drag-over');
  });
...
```javascript
const flowSteps = [
  { from: 'actor-1', to: 'actor-2', label: 'User clicks button', highlight: 'actor-1' },
  { from: 'actor-1', to: 'actor-2', label: 'Actor 1 sends message', highlight: 'actor-2', packet: true },
  { from: 'actor-2', to: 'external', label: 'Actor 2 calls API', highlight: 'actor-2', cloud: true },
];
...
window.flowNext = function() {
  ...
  if (step.packet) animatePacket(step.from, step.to);
...
<div class="arch-diagram">
  <div class="arch-zone arch-zone-browser">
    ...
    <div class="arch-component" data-desc="Injects UI into the web page"
...
Shows how different layers build on each other. Three tabs switch between views.
...
<div class="layer-demo">
  ...
  <div class="layer-viewport">
    ...
    <div class="layer" id="layer-js" style="display:none"></div>
  </div>
...
window.checkBugLine = function(el, isCorrect) {
  ...
  if (isCorrect) {
    ...
    feedback.textContent = 'Found it! The listener uses an async operation but does not return true.';
    feedback.className = 'bug-feedback show success';
  } else {
    el.classList.add('incorrect');
    feedback.textContent = 'Not this line — look for async timing issues...';
...
Same HTML/CSS/JS pattern as Multiple-Choice Quizzes, but with longer scenario descriptions. Wrap each question in a scenario context block:
...
<div class="callout callout-accent">
  ...
  <div class="callout-content">
    ...
    <p>This pattern — splitting responsibilities — is called "separation of concerns."</p>
  </div>
</div>
```

**Variants:**
- `callout-accent`: vermillion left border, light accent background (CS insights)
- `callout-info`: teal left border, light info background ("good to know")
- `callout-warning`: red left border, light error background (common mistakes)

---

## Pattern/Feature Cards

Grid of cards highlighting engineering patterns or key concepts.
...
<div class="pattern-cards">
  <div class="pattern-card" style="border-top: 3px solid var(--color-actor-1)">
    ...
    <p class="pattern-desc">Store results to avoid redundant work.</p>
  </div>
</div>
```
...
<div class="badge-list">
  <div class="badge-item">
    ...
    <span class="badge-desc">Save data between sessions</span>
  </div>
</div>
```
...
The most important accessibility feature for non-technical learners.

**HTML — mark up terms inline:**
```html
<p>The extension uses a
  <span class="term" data-definition="A service worker is a background script that runs independently of the web page.">service worker</span>
  to handle API calls.
</p>
```
...
.term-tooltip.flip { bottom: auto; top: calc(100% + 8px); }
...
**Rules:**
- Mark up EVERY technical term on first use in each module
...
- The dashed underline should be subtle but visible

---

## Visual File Tree

Use instead of paragraphs listing folder purposes.
  ...
  <div class="ft-folder open">
    ...
    <div class="ft-children">
      <div class="ft-folder">
        ...
        <span class="ft-desc">Backend endpoints</span>
      </div>
      <div class="ft-file">
...
For listing components, features, or concepts visually.
...
For sequences that would otherwise be a numbered paragraph list.
